...
REPO_DIR = os.environ.get("ACTIVEETF_REPO_DIR", BASE_DIR)
SAVE_PATH = os.path.join(REPO_DIR, "data")
KEEP_DAYS = 4
...
def read_all_holdings_from_zdsetf(etf_code):
    ...
    for table in tables:
        ...
        if set(required_columns).issubset(set(columns)):
            holding_df = table[required_columns].copy().rename(
                columns={
                    "代號": "股票代號",
                    "名稱": "股票名稱",
                    "股數": "持有股數",
...
def read_same_day_compare_from_zdsetf(etf_code, today_df):
    ...
    changed_df = pd.concat(compare_tables, ignore_index=True).rename(
        columns={
            "代號": "股票代號",
            "名稱": "股票名稱",
            "前日股數": "持有股數_昨日",
    ...
    return compare_df.sort_values(["變化", "權重(%)_今日"], ascending=[True, False])
